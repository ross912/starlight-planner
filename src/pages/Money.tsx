  ...
  const dailyBars = useMemo(
    () =>
      (stats?.daily ?? []).map((d) => ({
        ...
        expenseYuan: d.expense / 100,
        incomeYuan: d.income / 100,
      })),
    [stats],
  )
    ...
    <div className="space-y-5">
      ...
      {list.length > 0 && (
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          <section className="warm-card p-4 sm:p-5">
            <h3 className="font-semibold text-orange-950">每日收支</h3>
            <div className="mt-4 h-52">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dailyBars} barSize={8}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f5e0c3" vertical={false} />
                  <XAxis dataKey="day" tick={{ fontSize: 11, fill: '#a8a29e' }} tickLine={false} axisLine={{ stroke: '#f5e0c3' }} />
                  <YAxis tick={{ fontSize: 11, fill: '#a8a29e' }} tickLine={false} axisLine={false} width={44} />
                  <Tooltip
                    cursor={{ fill: 'rgba(251,191,36,0.12)' }}
                    contentStyle={{ borderRadius: 12, border: '1px solid #fde8cd', fontSize: 12 }}
                    formatter={(v: number, name: string) => [`¥${v.toLocaleString('zh-CN')}`, name === 'expenseYuan' ? '支出' : '收入']}
                    labelFormatter={(l) => `${month}-${String(l).padStart(2, '0')}`}
                  />
                  <Bar dataKey="expenseYuan" fill="#fb923c" radius={[3, 3, 0, 0]} />
                  <Bar dataKey="incomeYuan" fill="#34d399" radius={[3, 3, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </section>
      ...
      <section className="space-y-4">
        {groups.map(([date, txs]) => {
          ...
          return (
            <div key={date} className="warm-card overflow-hidden">
              <div className="flex items-center justify-between bg-orange-50/70 px-5 py-2.5 border-b border-orange-100/80">
                <span className="text-sm font-medium text-stone-700">
                  {d.getMonth() + 1}月{d.getDate()}日 周{WEEKDAYS[d.getDay()]}
                </span>
                <span className="text-xs text-stone-400">
                  {dayExpense > 0 && <span className="text-rose-400">-¥{fenToYuan(dayExpense)}</span>}
                  {dayExpense > 0 && dayIncome > 0 && ' · '}
                  {dayIncome > 0 && <span className="text-emerald-500">+¥{fenToYuan(dayIncome)}</span>}
                </span>
              </div>
              <ul>
                {txs.map((tx) => {
                  ...
                  return (
                    <li key={tx.id} className="group flex items-center gap-3 px-5 py-3 border-b border-orange-50 last:border-0 hover:bg-orange-50/40 transition">
                      <span className="flex h-9 w-9 items-center justify-center rounded-xl text-lg" style={{ background: `${cat?.color ?? '#a8a29e'}1f` }}>
                        {cat?.emoji ?? '📦'}
                      </span>
                      ...
                      <span className={`text-sm font-semibold ${tx.type === 'expense' ? 'text-rose-500' : 'text-emerald-600'}`}>
                        {tx.type === 'expense' ? '-' : '+'}¥{fenToYuan(tx.amount)}
                      </span>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="warm-btn-ghost !px-1.5 text-stone-400 hover:text-orange-600" onClick={() => setEditing(tx)} title="编辑">
                          <Pencil size={13} />
                        </button>
                        <button className="warm-btn-ghost !px-1.5 text-stone-400 hover:text-red-500" onClick={() => remove(tx)} title="删除">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </li>
